...
public class CoreOptions {
    ...
    @Documentation.Section(Documentation.Sections.EXPERT_CLASS_LOADING)
    public static final ConfigOption<String> ONLY_JOB_ALWAYS_PARENT_FIRST_LOADER_PATTERNS =
            ConfigOptions.key("classloader.only-job-parent-first-patterns.default")
                    .defaultValue("org.apache.hadoop.")
                    .withDeprecatedKeys("")
                    .withDescription(
                            "Useful only for child-first-cache. A (semicolon-separated) list of patterns that specifies which classes should always be"
                                    + " resolved through the parent ClassLoader first. A pattern is a simple prefix that is checked against"
                                    + " the fully qualified class name. This setting should generally not be modified. To add another pattern we"
                                    + " recommend to use \"classloader.only-job-parent-first-patterns.additional\" instead.");

    @Documentation.Section(Documentation.Sections.EXPERT_CLASS_LOADING)
    public static final ConfigOption<String>
            ONLY_JOB_ALWAYS_PARENT_FIRST_LOADER_PATTERNS_ADDITIONAL =
                    ConfigOptions.key("classloader.only-job-parent-first-patterns.additional")
                            .defaultValue("")
                            .withDescription(
                                    "Useful only for child-first-cache. A (semicolon-separated) list of patterns that specifies which classes should always be"
                                            + " resolved through the parent ClassLoader first. A pattern is a simple prefix that is checked against"
                                            + " the fully qualified class name. These patterns are appended to \""
                                            + ONLY_JOB_ALWAYS_PARENT_FIRST_LOADER_PATTERNS.key()
                                            + "\".");
    ...
    public static String[] getOnlyJobParentFirstLoaderPatterns(Configuration config) {
        String base = config.getString(ONLY_JOB_ALWAYS_PARENT_FIRST_LOADER_PATTERNS);
        String append = config.getString(ONLY_JOB_ALWAYS_PARENT_FIRST_LOADER_PATTERNS_ADDITIONAL);
        return parseParentFirstLoaderPatterns(base, append);
    }
